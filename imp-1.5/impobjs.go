...
func obj_FxSamplerWrapping(xn *xmlx.Node, n string) (obj *cdom.FxSamplerWrapping) {
	if (xn != nil) && (len(n) > 0) {
		xn = xcn(xn, n)
	}
	if xn != nil {
...
func objs_FxSamplerWrapping(xn *xmlx.Node, n string) (objs []*cdom.FxSamplerWrapping) {
	xns := xcns(xn, n)
...
func obj_FxWrapKind(xn *xmlx.Node, n string) (obj *cdom.FxWrapKind) {
	if (xn != nil) && (len(n) > 0) {
		xn = xcn(xn, n)
	}
	if xn != nil {
...
func objs_FxWrapKind(xn *xmlx.Node, n string) (objs []*cdom.FxWrapKind) {
	xns := xcns(xn, n)
	objs = make([]*cdom.FxWrapKind, len(xns))
	for i, xn := range xns {
